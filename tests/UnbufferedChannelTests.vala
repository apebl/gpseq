...
public abstract class UnbufferedChannelTests<G> : ChannelTests<G> {
	...
	protected UnbufferedChannelTests (string name) {
		base(name, 0);
	}

	protected override Channel<G> create_channel (int cap) {
		assert(cap == 0);
		return Channel.bounded<G>(0);
	}

	public class Int : UnbufferedChannelTests<int> {
		public Int () {
			base("channel:unbuffered<int>");
		}
	...
	public class NullableInt : UnbufferedChannelTests<int?> {
		public NullableInt () {
			base("channel:unbuffered<int?>");
		}
	...
	public class String : UnbufferedChannelTests<string> {
		public String () {
			base("channel:unbuffered<string>");
		}
	...
	public class NullableString : UnbufferedChannelTests<string?> {
	...
	public class ObjTests : UnbufferedChannelTests<Obj> {
		private static int64 objects;

		public ObjTests () {
			base("channel:unbuffered<Obj>");
		}
